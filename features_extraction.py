...
#sys.path.insert(0, '/home/shreyas/anaconda3/lib/python3.7/site-packages/bs4/builder')
...
#total no minus 2 gives the exact value required....
    if len(list)<=3:#3-2=1dot ....legitimate site  X . X
        return 1
    elif len(list)==4:#4-2=2 dots ....suspicious X . . X
        return 0
    else:
        return -1#more than 4 dots ....pakka malicious X . . . . X
...
def domain_registration_length(domain): #DONE 
    ...
    today = time.strftime('%Y-%m-%d') #strftime() converts a time input into a string output.
    today = datetime.strptime(today, '%Y-%m-%d') #strptime() converts a text input into a time output.
...
def url_of_anchor(wiki, soup, domain):
    #with open('/opt/lampp/htdocs/BE/markup_facebook_1.txt', 'rb') as file:
     #   soup_string=file.read()

    #soup = BeautifulSoup(soup_string, 'html.parser')
    #wiki="https://www.facebook.com"
    #domain="facebook.com"
    ...
    # 2nd condition was 'JavaScript ::void(0)' but we put JavaScript because the space between javascript and :: might not be
        ...
        if "#" in a['href'] or "javascript" in (a['href'] or a['href'].lower()) or "mailto" in (a['href'] or a['href'].lower()) or not (wiki in a['href'] or domain in a['href']):
            unsafe = unsafe + 1
        i = i + 1
        # print a['href']
...
def main(url):    
    ...
    with open('/opt/lampp/htdocs/BE/newmarkupnew5google.txt','rb') as file:
        soup_string=file.read()
	
    soup = BeautifulSoup(soup_string,'html.parser')#'html5lib')   
    #soup = BeautifulSoup(page.text,'html.parser')#'html5lib')   
    ...
    #1status.append(url_length(url))
    #2status.append(shortening_service(url))
    #3status.append(having_at_symbol(url))
    #4status.append(double_slash_redirecting(url))
    status.append(prefix_suffix(hostname))
    status.append(having_sub_domain(url))
    #7 status.append(sslfinal_state(url))
    ...
    try:
        ...
        dns=-1
    if dns==-1:
        status.append(-1)#because not all sites are registered in dns
    else:
        status.append(domain_registration_length(domain))#8

    #9status.append(favicon(url,soup, hostname))
    #11status.append(https_token(url))
    status.append(request_url(url, soup, hostname))#12
    status.append(url_of_anchor(url, soup, hostname))#13
    status.append(links_in_tags(url,soup, hostname))#14
    status.append(sfh(url,soup, hostname))#15
    #16status.append(submitting_to_email(soup))
    """
    if dns == -1:
        status.append(-1)
    else:
    ...
    #22status.append(iframe(soup))
    
    if dns == -1:#not present in dns server
        status.append(-1)
    else:
        status.append(age_of_domain(domain))#23

    status.append(dns)#24

    #status.append(web_traffic(url))#25
    status.append(google_index(url))#27
    #29status.append(statistical_report(url,hostname))
...
if __name__ == "__main__":
    ...
    #url="https://proassessments.co.uk/ASB/nz/index.htm"
    main(url)
